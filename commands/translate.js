...
export default {
  ...
  async execute({ argsText, raw, message, senderJid, reply, ai }) {
    ...
    if (!text) throw new Error('Send text after .translate or reply to a text message with .translate');
    const result = await ai.translate({ text, sessionId: message.sessionId, chatJid: message.chatJid, senderJid });
