...
export class NightAgent {
  ...
  async run({request,sessionId,chatJid,senderJid,raw=null}={}){
    const catalog=`Allowed actions: search(query), summarize(text), translate(text), transcribe(), vision(), sticker(), note(text), todo(text), reminder(text), anime(mode,query), manga(mode,query), meme(dark), qr(text), image(prompt), table(title,columns,rows), save(text), status(text), answer(text). Use $last in a later string argument to reference the previous action output. Media actions operate on the replied WhatsApp media. Maximum 6 actions. Never invent an action outside this list.`;
    ...
    for(const action of actions){const type=String(action?.type||'').toLowerCase();let out;
      ...
      else if(type==='vision'){const r=await this.features.vision(raw);out=[r.caption,r.text,r.tags?.length?`Tags: ${r.tags.join(', ')}`:''].filter(Boolean).join('\n');}
      ...
      else if(type==='status'){await this.features.postStatus({sessionId,text:replaceLast(action.text,last),raw});out='Status posted.';}
