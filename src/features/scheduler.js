...
export class FeatureScheduler {
  ...
  async tickWatches(now=Date.now()){
    for(const watch of this.store.listWatches(100)){
      try{
        ...
        if(watch.last_value!=null && current!==watch.last_value){
          await this.sessions.sendViaSession(watch.session_id,watch.chat_jid,{text:`Watch update: ${watch.query}\n\n${current}`});
        }
  ...
  async #resolveWatch(watch){
    if(watch.kind==='anime'){
      const rows=await animeList('search',watch.query); const x=rows[0]; if(!x)return null; return JSON.stringify({title:x.title,status:x.status,episodes:x.episodes,score:x.score});
    }
    if(watch.kind==='manga'){
      const rows=await mangaList('search',watch.query); const x=rows[0]; if(!x)return null; return JSON.stringify({title:x.title,status:x.status,chapters:x.chapters,score:x.score});
    }
    if(watch.kind==='web'){
      const r=await this.ai.search({text:`Check the current state of: ${watch.query}. Return a compact factual state summary only.`,sessionId:'watch',chatJid:`watch:${watch.id}`});
